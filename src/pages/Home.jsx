...
const handleBooking = async () => {
  ...
  if (!selected) return alert("Please select a bus first.");
  if (chosen.length === 0) return alert("Please select at least one seat.");
  ...
  try {
    const res = await fetch("/api/bookings", {
      ...
      body: JSON.stringify({
        ...
        busId: selected._id,
        seatsBooked: chosen.length,
        totalPrice: chosen.length * selected.price
      })
    });
    const data = await res.json();
    if (res.ok) {
      alert("✅ Ticket booked and E-Ticket sent to your email!");
      setChosen([]); // Clear seats after successful booking
    } else {
...
<div><Card className="rounded-[2rem] bg-white p-5 text-slate-950"><div className="flex justify-between"><div><p className="text-xs font-bold tracking-widest text-cyan-700">{selected?.operator?.toUpperCase() || "SEAT STUDIO"}</p><h2 className="text-3xl font-black">Choose seats</h2></div><span className="grid h-12 w-12 place-items-center rounded-2xl bg-slate-950 text-cyan-300"><Ticket/></span></div><div className="mt-5 rounded-3xl bg-slate-100 p-5"><div className="mx-auto mb-4 w-28 rounded-t-2xl bg-slate-300 py-2 text-center text-xs font-bold">DRIVER</div><div className="grid grid-cols-4 gap-2">{seats.map((s,i)=><button key={s.id} onClick={()=>toggle(s)} disabled={s.booked} className={`h-11 rounded-xl font-black ${i%4===1?"mr-5":""} ${s.booked?"bg-slate-300 text-slate-500":chosen.includes(s.id)?"bg-cyan-500 text-white":"bg-emerald-200 text-emerald-900"}`}>{s.id}</button>)}</div></div><div className="mt-5 rounded-3xl border p-5"><div className="flex justify-between"><div><p className="font-black">{selected?.from || "..."} to {selected?.to || "..."}</p><p className="text-sm text-slate-500">Seats: {chosen.join(", ")||"None"}</p></div><div className="text-right"><p className="text-xs text-slate-500">TOTAL</p><p className="text-2xl font-black">LKR {(chosen.length*(selected?.price || 0)).toLocaleString()}</p></div></div><Button onClick={handleBooking} disabled={isBooking || !selected || chosen.length === 0} className="mt-4 flex w-full items-center justify-center gap-2 rounded-2xl bg-slate-950 py-4 font-black text-white"><CreditCard/> {isBooking ? "Processing..." : "Pay & Book"}</Button></div></Card><div className="mt-4 grid grid-cols-3 gap-3">{[[LocateFixed,"Live GPS"],[ShieldCheck,"Safe Pay"],[QrCode,"QR Ticket"]].map(([I,t])=><div key={t} className="rounded-2xl bg-white/10 p-4 text-center"><I className="mx-auto text-cyan-300"/><p className="mt-2 text-sm font-bold">{t}</p></div>)}</div></div></section>
